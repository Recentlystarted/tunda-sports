...
import { Plus, Trash2, Users, Phone, Mail, Calendar, MapPin, Trophy, DollarSign, AlertCircle, Crown, Target } from 'lucide-react'
...
export default function GroupRegistration({ tournament }: GroupRegistrationProps) {
  ...
  const [formData, setFormData] = useState<TeamRegistrationData>({
    ...
    emergencyPhone: '',
    agreeToTerms: false,
    tournamentExperience: 'INTERMEDIATE',
    teamStrengths: []
  })
  ...
  const toggleTeamStrength = (strength: string) => {
    setFormData(prev => ({
      ...prev,
      teamStrengths: prev.teamStrengths.includes(strength)
        ? prev.teamStrengths.filter(s => s !== strength)
        : [...prev.teamStrengths, strength]
    }))
  }
    ...
    try {
      const registrationData = {
        ...
        additionalInfo: {
          ...
          preferredGroup: formData.preferredGroup,
          tournamentExperience: formData.tournamentExperience,
          teamStrengths: formData.teamStrengths,
        }
      }
  ...
  const teamStrengthOptions = [
    { key: 'BATTING', label: 'Strong Batting' },
    { key: 'BOWLING', label: 'Strong Bowling' },
    { key: 'FIELDING', label: 'Excellent Fielding' },
    { key: 'ALL_ROUNDERS', label: 'All Rounders' },
    { key: 'EXPERIENCED', label: 'Experienced Players' },
    { key: 'YOUNG_TALENT', label: 'Young Talent' }
  ]
    ...
    <Card>
      <CardHeader>
        <CardTitle className="flex items-center gap-2">
          ...
          Team Registration - Group Tournament
        </CardTitle>
        <CardDescription>
          Register your team for this group-based tournament. Teams will be organized into groups based on preferences and strengths.
        </CardDescription>
      </CardHeader>
      <CardContent>
        <form onSubmit={handleSubmit} className="space-y-8">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div>
                ...
                <Select
                  ...
                  onValueChange={(value) => setFormData(prev => ({ ...prev, preferredGroup: value }))}
                >
                  <SelectTrigger>
                    <SelectValue placeholder="Select preferred group (optional)" />
                  </SelectTrigger>
                  <SelectContent>
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-4">
              <div>
                <Label htmlFor="tournamentExperience">Tournament Experience Level</Label>
                <Select
                  value={formData.tournamentExperience}
                  onValueChange={(value) => setFormData(prev => ({ ...prev, tournamentExperience: value }))}
                >
                  <SelectTrigger>
                    <SelectValue />
                  </SelectTrigger>
                  <SelectContent>
                    ...
                    <SelectItem value="INTERMEDIATE">Intermediate (Few tournaments)</SelectItem>
                    <SelectItem value="ADVANCED">Advanced (Many tournaments)</SelectItem>
                    <SelectItem value="PROFESSIONAL">Professional (Regular participation)</SelectItem>
                  </SelectContent>
                </Select>
              </div>
              ...
              <div>
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-2 mt-2">
                  {teamStrengthOptions.map((strength) => (
                    <div key={strength.key} className="flex items-center space-x-2">
                      <Checkbox
                        id={strength.key}
                        checked={formData.teamStrengths.includes(strength.key)}
                        onCheckedChange={() => toggleTeamStrength(strength.key)}
                      />
                      <Label htmlFor={strength.key} className="text-sm">
                        {strength.label}
                      </Label>
                    </div>
                  ))}
